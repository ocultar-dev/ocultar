...
| `OCU_LOG_LEVEL` | Log level for all 4 binaries (`apps/proxy`, `apps/sombra`, `apps/slm-engine`, `services/refinery/cmd`): `debug`\|`info`\|`warn`\|`error`, default `info`. All logging is structured JSON to stderr via `log/slog`; `slog.SetDefault` also redirects any remaining `log.Printf`-style calls through the same JSON handler. |
