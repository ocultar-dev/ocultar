.PHONY: all build test clean sync-work

all: sync-work build test

sync-work:
	go work sync

build:
	CGO_ENABLED=1 go build ./internal/pii/...
	CGO_ENABLED=1 go build ./services/refinery/...
	CGO_ENABLED=1 go build ./services/vault/...
	CGO_ENABLED=1 go build ./apps/proxy/...
	CGO_ENABLED=1 go build ./apps/sombra/...
	CGO_ENABLED=1 go build ./apps/slm-engine/...

test:
	CGO_ENABLED=1 go test ./internal/pii/...
	CGO_ENABLED=1 go test ./services/refinery/...
	CGO_ENABLED=1 go test ./services/vault/...
	CGO_ENABLED=1 go test ./apps/proxy/...
	CGO_ENABLED=1 go test ./apps/sombra/...
	CGO_ENABLED=1 go test ./apps/slm-engine/...

clean:
	go clean -cache
	rm -rf dist/

release:
	@echo "Building release artifacts..."
	bash tools/scripts/release.sh
	@echo "Tagging v1.15.0..."
	git tag -a v1.15.0 -m "Release v1.15.0"
	@echo "Pushing tag to origin..."
	git push origin v1.15.0
	@echo "Done! Please create the release on GitHub: https://github.com/Edu963/ocultar/releases/new?tag=v1.15.0"
	@echo "Make sure to attach the .zip and .tar.gz files from the dist/ folder to the release!"


